...
class Node {
	...
	heuristic(other) {
	  return this.boundObject.position.distanceToSquared(other.boundObject.position);
	}
	...
	findPath(goal, form) {
		...
		while (traversal.length > 0) {
			traversal.sort((a, b) => (a._cost + a.heuristic(goal)) - (b._cost + b.heuristic(goal)));
		  ...
		  for (let neighbor of current.neighbors) {
				...
				if (neighbor._searchID != sID || neighborNewCost < neighbor.cost) {
					...
					if (!neighbor.isOccupied() && (formAgnostic || neighbor.allowedForms.includes(form))) traversal.push(neighbor);
		...
		if (found) {
			let previous = goal.isOccupied() || (!formAgnostic && !goal.allowedForms.includes(form)) ? goal._previous : goal;
			const result = [];
			while (previous) {
...
class BoundObject {
	...
	equals(other) {
		if (other) return this == other || this.position.manhattanDistanceTo(other.position) < 0.1;
	}
}
...
class Level {
  constructor(levelInfo, world) {
		...
		for (let i = 0; i < levelInfo.ground.length; i++) {
			...
			for (let j = 0; j < levelInfo.ground[i].length; j++) {
				if (levelInfo.ground[i][j]) {
					const node = new Node(new BoundObject({
						x: i * levelParameters.tileSize,
						y: j * levelParameters.tileSize,
						groundHeight: levelParameters.tileSize * levelParameters.groundHeight
					}));
					...
					if (i > 0 && levelInfo.ground[i - 1].length > i && levelInfo.ground[i - 1][j]) {
				...
				if (waypoint instanceof Array) {
					if (
					  waypoint[0] >= 0 &&
						...
						console.warn(`Lilypad path ${path}: ${waypoint} doesn't yield to any ground tile`);
					}
				}
				else {
					if (0 <= waypoint && waypoint < lilypadNodes.length) {
						...
						console.warn(`Lilypad path ${path}: <${waypoint}> doesn't yield to any lilypad`);
					}
				}
	...
	move(node) {
		...
		if (path) {
			const result = this.world.frogGirl.locomote(path.map(
			  waypoint => new Vector3(waypoint.boundObject.position.x, waypoint.boundObject.groundHeight, waypoint.boundObject.position.z)
			));
